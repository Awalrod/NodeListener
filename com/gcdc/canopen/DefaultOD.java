...
System.out.println("WARNING: hardcoding default rpdo 3&4 node ids to 0x33");
	...
	// cobid - rxpdo value of pdo to look for
	// index - range 0x00 - 0xff index to place this entry adds 0x1400 to entry and 0x1600 to mapping
	// targetIndex entry where data is placed (typically starts at 0x6200)
	// numSamples - number of subindexs (samples)
	// numBits - number of bits per sample (subindex)
	public static void insertRxpdo(ObjectDictionary objDict, int cobid, int index, int targetIndex, int numSamples, int numBits){
		{//targetIndex <<16 | numSamples<<8 | numBits
			if(index> 0x100){
				index &= 0xFF;
			...
			int[] maps = new int[numSamples];
			OdEntry od = new OdEntry(targetIndex, "Analog Output Offset");
			objDict.insert(od);
