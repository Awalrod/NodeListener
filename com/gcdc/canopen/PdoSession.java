...
debugPrint("PdoSession cobId match "+  String.format("%04X", msg.id).toUpperCase());
		...
		debugPrint("PdoSession.processMessage() numMaps: " + Integer.toString(numMaps));
		for(int i=1;i<=numMaps; i++)
		{
			...
			debugPrint("PdoSession.processMessage() index , subindex: 0x"+ String.format("%04x", index).toUpperCase()+ String.format("%02x",sub).toUpperCase());
